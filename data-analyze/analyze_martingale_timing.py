...
class TimeBasedStreakAnalyzer:
    ...
    def find_losing_streaks(
        self, sessions: Dict[int, List[Tuple]], min_length: int = 8
    ) -> List[Dict]:
        """
    ...
    def analyze_all(self) -> Dict:
        ...
        results["losing_daily_hourly"] = self.analyze_hourly_frequency_per_day(
            losing_streaks
        )
        ...
        if losing_streaks:
            ...
            print(
                f"  Total losing streak occurrences across all days: {len(losing_streaks)}"
            )
    ...
    def generate_report(self, results: Dict, output_path: str):
        ...
        lines.append(
            f"Hot Streaks per Day: {len(hot_streaks) / date_range['total_days']:.2f}"
        )
        lines.append(
            f"Losing Streaks per Day: {len(losing_streaks) / date_range['total_days']:.2f}"
        )
        ...
        lines.append("=" * 120)
        lines.append(
            f"{'Hour':<6} {'Hot Streaks':<25} {'Losing Streaks':<25} {'Recommendation':<30}"
        )
        lines.append("-" * 120)
        ...
        for hour in range(24):
            ...
            losing_info = (
                f"{losing_stats['total_count']:3d} total ({losing_avg:.2f}/day)"
            )

            lines.append(
                f"{hour:02d}:00  {hot_info:<25} {losing_info:<25} {recommendation:<30}"
            )
        ...
        # Sort dates
        all_dates = sorted(
            set(list(hot_daily_hourly.keys()) + list(losing_daily_hourly.keys()))
        )
        ...
        for date_str in all_dates:
            ...
            lines.append(
                f"  {'Hour':<6} {'Hot Streaks':<15} {'Losing Streaks':<15} {'Status':<20}"
            )
            lines.append("  " + "-" * 70)
            ...
            for hour in all_hours:
                ...
                lines.append(
                    f"  {hour:02d}:00  {hot_count:<15} {losing_count:<15} {status:<20}"
                )
        ...
        for rank, streak in enumerate(hot_streaks[:20], 1):
            ...
            lines.append(
                f"  Time: {streak['start_time'].strftime('%Y-%m-%d %H:%M:%S')} → {streak['end_time'].strftime('%H:%M:%S')}"
            )
            lines.append(
                f"  Day: {streak['start_time'].strftime('%A')}, Hour: {streak['start_time'].hour:02d}:00"
            )
            lines.append(
                f"  Average: {streak['avg_multiplier']:.2f}x | Range: {streak['min_multiplier']:.2f}x - {streak['max_multiplier']:.2f}x"
            )
        ...
        lines.append("-" * 120)
        lines.append(
            f"{'Rank':<6} {'Hour':<8} {'Hot/Day':<12} {'Losing/Day':<12} {'Score':<10} {'Recommendation'}"
        )
        lines.append("-" * 120)
        ...
        for rank, (hour, hot_avg, losing_avg, score) in enumerate(best_hours[:10], 1):
            ...
            lines.append(
                f"{rank:<6} {hour:02d}:00  {hot_avg:<12.2f} {losing_avg:<12.2f} {score:<10.2f} {rec}"
            )
        ...
        for rank, (hour, hot_avg, losing_avg, score) in enumerate(
            reversed(best_hours[-10:]), 1
        ):
            if score < -0.5:
                rec = "⚠️  AVOID"
            elif score < 0:
    ...
    def create_visualizations(self, results: Dict, output_dir: str):
        ...
        self.plot_daily_hourly_heatmap(
            results, "hot", f"{output_dir}/hot_streak_hourly_heatmap.png"
        )

        # 2. Heatmap: Hour x Day for losing streaks
        self.plot_daily_hourly_heatmap(
            results, "losing", f"{output_dir}/losing_streak_hourly_heatmap.png"
        )

        # 3. Combined comparison
        self.plot_hourly_comparison(
            results, f"{output_dir}/streak_hourly_comparison.png"
        )

        # 4. Best hours ranking
        self.plot_best_hours(results, f"{output_dir}/best_hours_for_martingale.png")

        # 5. Daily overview
        self.plot_daily_overview(results, f"{output_dir}/daily_streak_overview.png")

        # 6. Streak characteristics
        self.plot_streak_characteristics(
            results, f"{output_dir}/streak_characteristics.png"
        )

        print("✓ All visualizations generated!")

    def plot_daily_hourly_heatmap(
        self, results: Dict, streak_type: str, output_path: str
    ):
        ...
        ax.set_yticklabels(
            [f"{d} ({datetime.strptime(d, '%Y-%m-%d').strftime('%a')})" for d in dates]
        )
    ...
    def plot_hourly_comparison(self, results: Dict, output_path: str):
        ...
        ax1.set_title(
            "Hot vs Losing Streaks - Average Frequency per Hour",
            fontsize=14,
            fontweight="bold",
        )
        ax1.set_xticks(x)
        ...
        bars3 = ax2.bar(
            x, net_advantage, color=colors, alpha=0.8, edgecolor="black", linewidth=0.5
        )
        ...
        ax2.set_title(
            "Martingale Strategy Advantage by Hour (Positive = Good, Negative = Bad)",
            fontsize=14,
            fontweight="bold",
        )
        ax2.set_xticks(x)
    ...
    def plot_best_hours(self, results: Dict, output_path: str):
        ...
        # Labels
        hour_labels = [
            f"{h['hour']:02d}:00 (H:{h['hot_avg']:.1f} L:{h['losing_avg']:.1f})"
            for h in hours_data
        ]
        ax.set_yticks(y_pos)
    ...
    def plot_daily_overview(self, results: Dict, output_path: str):
        ...
        bars3 = ax2.bar(
            x, ratios, color=colors, alpha=0.8, edgecolor="black", linewidth=0.5
        )
        ax2.axhline(
            y=1, color="black", linestyle="--", linewidth=1.5, label="Equal (1:1)"
        )

        ax2.set_xlabel("Date", fontsize=11, fontweight="bold")
        ax2.set_ylabel("Hot/Losing Ratio", fontsize=11, fontweight="bold")
        ax2.set_title(
            "Daily Hot/Losing Ratio (>1 = More Hot, <1 = More Losing)",
            fontsize=13,
            fontweight="bold",
        )
        ax2.set_xticks(x)
    ...
    def plot_streak_characteristics(self, results: Dict, output_path: str):
        ...
        fig.suptitle(
            "Streak Characteristics Comparison", fontsize=16, fontweight="bold"
        )
        ...
        bp = ax2.boxplot(
            [hot_avgs, losing_avgs],
            labels=["Hot Streaks", "Losing Streaks"],
            patch_artist=True,
        )
        ...
        hot_durations = [
            (s["end_time"] - s["start_time"]).total_seconds() / 60 for s in hot_streaks
        ]
        losing_durations = [
